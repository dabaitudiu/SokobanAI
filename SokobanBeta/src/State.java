...
public class State {
    ...
    public State(State s) {
        ...
        this.sequence = new ArrayList<>(s.sequence);
    }
    ...
    public void move(int ax, int ay, int bx, int by, String s, boolean verbose) {
        ...
        if (!walls.contains(attempt)) {
            if (!boxes.contains(attempt) || !boxes.contains(newbox) && !walls.contains(newbox)) {
                ...
                Graph tmpGraph = new Graph(rows, cols, walls,boxes,storages,player);
                sequence.add(tmpGraph);
                State cur = new State(walls,new HashSet<>(boxes),new HashSet<>(storages),attempt,move + s, rows, cols,verbose, deadlocks, new ArrayList<>(sequence));
